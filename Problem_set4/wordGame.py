...
WORDLIST_FILENAME = "c:/users/Monte/6.00.1x Files/ps4/words.txt"
...
def getWordScore(word, n):
    ...
    GWScore = 0
    for L in word:
        GWScore += SCRABBLE_LETTER_VALUES[L]
    GWScore = GWScore * len(word)
    if len(word) == n:
        GWScore += 50
    return GWScore
...
def isValidWord(word, hand, wordList):
    ...
    leftHand = hand.copy()
    for L in word:
        if L not in leftHand or leftHand[L] == 0:
            return False
        else:
            leftHand[L] -= 1
    if word in wordList:
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    print(exitMsg + "Total score: " + str(totalScore) + " points.")
